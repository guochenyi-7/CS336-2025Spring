...
from cs336_basics.utils import cross_entropy_loss, get_batch, save_checkpoint, load_checkpoint
from cs336_basics.optimizer import MyAdamW, lr_cosine_schedule, gradient_clipping
...
def run_linear(
    ...
    """
    model = Linear(d_in, d_out, weights.device, weights.dtype)
    model.load_state_dict({"weight": weights})
    with torch.inference_mode():
        out_put = model(in_features)
...
def run_embedding(
    ...
    """
    model = Embedding(vocab_size, d_model, weights.device, weights.dtype)
    model.load_state_dict({"weight": weights})
...
def run_swiglu(
    ...
    model = SwiGLU(d_model, d_ff, w1_weight.device, w1_weight.dtype)
...
def run_rmsnorm(
    ...
    """
    model = RMSNorm(d_model, eps, weights.device, weights.dtype)
    model.load_state_dict({"weight": weights})
