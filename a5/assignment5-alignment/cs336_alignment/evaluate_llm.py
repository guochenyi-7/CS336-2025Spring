...
def evaluate_vllm(
    ...
    for i, output_obj in enumerate(outputs):
        generated_text = "<think>" + output_obj.outputs[0].text
...
if __name__ == "__main__":
    ...
    local_model_path = "/home/prey/.cache/huggingface/hub/models--Qwen--Qwen2.5-Math-1.5B/snapshots/4a83ca6e4526a4f2da3aa259ec36c259f66b2ab2"
    # 设置采样参数
    ...
    #提取出问题和答案
    prompts = [PROMPTS_TEMPLATE.format(question=item["question"]) for item in data]
    ground_truths = [item["answer"] for item in data]

    llm = LLM(model=local_model_path, gpu_memory_utilization=0.8)
